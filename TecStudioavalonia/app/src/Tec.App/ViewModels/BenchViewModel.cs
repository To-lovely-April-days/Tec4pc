...
public sealed class BenchViewModel : ViewModelBase
{
    ...
    public void Reload()
    {
        ...
        ChannelRows.Clear();
        foreach (var ch in _ws.Channels) ChannelRows.Add(new ChannelRowViewModel(ch));
...
{
    public ChannelRowViewModel(Channel ch)
    {
        Channel = ch;
    ...
    public string Host => Channel.HostInstanceId + " · 孔 " + (Channel.Well + 1);
